...
export const ProgressBar = ({ userId }: { userId: number }) => {
  ...
  useEffect(() => {
    ...
    // 처음 렌더링될 때 한 번만 진행률 계산
    const { percentage } = calculateProgress(blockInfo);
    setProgress(percentage);
  }, [blockInfo]); // blockInfo가 변경될 때만 실행 (한 번만 실행됨)
...
export const DurationElapsedPercentage = (
  ...
  const blockInfo = useBlockInfo(userId); // 사용자 차단 정보 가져오기

  // blockInfo가 변경될 때만 값 계산
  return useMemo(() => {
  ...
  }, [blockInfo]); // blockInfo가 변경될 때만 실행됨
};
