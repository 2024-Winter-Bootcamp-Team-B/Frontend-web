...
useEffect(() => {
  ...
  const startInertia = (index: number) => {
    ...
    const animate = () => {
      ...
      if (Math.abs(currentVelocity.dx || 0) >= 0.1 || Math.abs(currentVelocity.dy || 0) >= 0.1) {
      ...
      <div className="flex gap-8">
        <div className="flex flex-col grow w-full justify-between">
          ...
          <div className="font-semibold text-4xl">
            ...
            <p>목표시간 {totalDuration}, 달성시간 {elapsedTime}</p>
          </div>

          {/* ProgressBar에 user_id 전달 */}
          {user_id && <ProgressBar userId={user_id} />}

          <div className={`jail-container w-full flex ${isJailOpen ? 'jail-open' : ''}`} ref={jailRef}>
